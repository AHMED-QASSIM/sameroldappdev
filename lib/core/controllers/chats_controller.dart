...
class ChatsController extends GetxController {
  ...
  Future<void> getMessages(String? userId) async {
    ...
    _currentSubscription
        ?.cancel(); // Cancel the current subscription if it exists
  ...
  addChat(user_id, user_name, user_image, user_phone, type, subscription,
      {message, imageType, fileName}) async {
    if (type == "text") {
      ChatServices()
          .sendMessage(user_id, user_name, user_image, _currentAuthUserId,
              message, "text", user_phone, subscription)
          .then((value) {});
    }
    ...
    if (type == "image") {
      ...
      await ref.putFile(message).then((p0) async {
        ref.getDownloadURL().then((value) async {
          ...
          ChatServices()
              .sendMessage(user_id, user_name, user_image, _currentAuthUserId,
                  value, "image", user_phone, subscription)
              .then((value) {});
        });
      });
    }
    ...
    if (type == "file") {
      ...
      await ref.putFile(message).then((p0) async {
        ref.getDownloadURL().then((value) async {
          ...
          ChatServices()
              .sendMessage(user_id, user_name, user_image, _currentAuthUserId,
                  value, "file", user_phone, subscription,
                  fileName: fileName)
              .then((value) {});
        });
      });
      ...
      try {
        ...
        await ref.putFile(file).then((p0) async {
          ref.getDownloadURL().then((value) async {
            ...
            ChatServices()
                .sendMessage(user_id, user_name, user_image, _currentAuthUserId,
                    value, "voice", user_phone, subscription)
                .then((value) {});
          });
        });
      } catch (e) {
